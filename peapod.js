...
Peapod.prototype.makeLoginRequest = function(cb) {
    var options = {
      ...
      headers: {
        ...
        'referer': 'http://www.peapod.com/shop/auth/index.jhtml'
      },
      json: {
...
Peapod.prototype.detail = function (items, callback) {
    var url = 'v3.0/user/products/' + items.join(',');
    this.request('GET', url, {}, function (err, body, response) {
        if (err) {
            callback(err, null);
        } else {
...
Peapod.prototype.search = function (query, callback) {
    var params = {
        'facet': 'singleRootCat,brands,nutrition,specials,newArrivals,privateLabel',
    ...
    this.request('GET', 'v3.0/user/products', params, function (err, body, response) {
        if (err) {
            callback(err, null);
        } else {
            var json = JSON.parse(body);
